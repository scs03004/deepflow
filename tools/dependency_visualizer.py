...
class DependencyAnalyzer:
    ...
    def analyze_project(self) -> DependencyGraph:
        ...
        return DependencyGraph(
            nodes=self.nodes,
            edges=[(u, v, self.graph[u][v].get('type', 'import')) 
            ...
            if isinstance(node, ast.Import):
                ...
                if node.module:
                    ...
                    if module.startswith('.'):
                        ...
                        imports['internal'].append(self._resolve_relative_import(
                            module, file_path))
    ...
    def _is_internal_module(self, module: str) -> bool:
        ...
        for i in range(len(parts)):
            ...
            if partial_path.with_suffix('.py').exists() or \
               (partial_path / '__init__.py').exists():
                return True
        return False
    ...
    def _calculate_metrics(self) -> Dict[str, int]:
        ...
        return {
            ...
            'high_risk_files': sum(1 for node in self.nodes.values() 
...
class DependencyVisualizer:
    ...
    def generate_text_tree(self) -> str:
        ...
        root_nodes = [name for name, node in self.graph.nodes.items() 
                     if not node.imports]
        
        if not root_nodes:
            # If no true roots, pick nodes with fewest dependencies
            root_nodes = sorted(self.graph.nodes.keys(), 
                              key=lambda x: len(self.graph.nodes[x].imports))[:3]
    ...
    def _build_tree_recursive(self, tree: Tree, node_name: str, visited: Set[str], max_depth: int = 5):
        ...
        for imported in node.imports:
            if imported in self.graph.nodes:
                risk_color = {
                    ...
                    'MEDIUM': 'yellow', 
                    'LOW': 'green'
                }.get(self.graph.nodes[imported].risk_level, 'white')
    ...
    def generate_html_interactive(self, output_path: str):
        ...
        node_trace = go.Scatter(
            ...
            customdata=[[self.graph.nodes[name].risk_level,
                        self.graph.nodes[name].lines_of_code,
                        len(self.graph.nodes[name].imports),
                        len(self.graph.nodes[name].imported_by)] for name in node_names],
            marker=dict(
                size=[max(10, min(50, self.graph.nodes[name].lines_of_code / 10)) 
                     for name in node_names],
                color=[node_colors[self.graph.nodes[name].risk_level] 
                      for name in node_names],
    ...
    def generate_risk_heatmap(self, output_path: str):
        ...
        for name, node in self.graph.nodes.items():
            nodes_data.append({
                ...
                'Risk Score': len(node.imported_by) * 2 + len(node.imports) + (node.lines_of_code / 100)
            })
    ...
    def generate_summary_report(self) -> str:
        ...
        # High-risk files
        high_risk = [name for name, node in self.graph.nodes.items() 
                    if node.risk_level == 'HIGH']
        ...
        if high_risk:
            ...
            for file in high_risk[:10]:  # Top 10
                ...
                report.append(f"- **{file}** ({node.lines_of_code} LOC, "
        ...
        if self.graph.external_dependencies:
            ...
            sorted_deps = sorted(self.graph.external_dependencies.items(), 
